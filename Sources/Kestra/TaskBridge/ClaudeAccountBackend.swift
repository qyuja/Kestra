...
enum ClaudeAccountBackend {
    ...
    static let vaultService = KestraAppIdentity.bundleIdentifier + ".claude-accounts"
    static let legacyVaultService = KestraAppIdentity.legacyBundleIdentifier + ".claude-accounts"
    static var configFile: URL { FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".claude.json") }

    static func readVaultSecret(account: String) throws -> Data? {
        try readSecret(service: vaultService, account: account)
            ?? readSecret(service: legacyVaultService, account: account)
    }

    static func writeVaultSecret(_ data: Data, account: String) throws {
        try writeSecret(data, service: vaultService, account: account)
    }
