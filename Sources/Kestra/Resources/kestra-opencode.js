...
export const KestraPlugin = async ({ directory }) => {
  const folder = join(homedir(), "Library/Application Support/com.kiannest.kestra/opencode-events");
  ...
  const record = (sessionID, kind, fields = {}) => {
    ...
    queue = queue.then(async () => {
      ...
      console.error("Kestra: could not record OpenCode task event");
    });
    return queue;
  };
  return {
