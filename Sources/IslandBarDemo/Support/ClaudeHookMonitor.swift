...
final class ClaudeHookMonitor {
    static let eventsDirectory = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Application Support/com.kiannest.islandbar/claude-events")
    ...
    init(directory: URL = ClaudeHookMonitor.eventsDirectory, provider: AIProvider = .claude) {
    ...
    static func receiveEvent(provider: AIProvider = .claude) {
        ...
        do {
            ...
            FileHandle.standardError.write(Data("AI Task Radar: failed to record Claude hook event\n".utf8))
        }
    }
    ...
    static func installHooks(configDirectory: URL = ClaudeHookMonitor.configDirectory, argument: String = "--claude-hook", events: [String] = ["UserPromptSubmit", "PreToolUse", "Stop", "StopFailure", "SessionEnd"], filename: String = "settings.json") throws {
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            ...
            object = existing
            try data.write(to: configDirectory.appendingPathComponent("settings.islandbar-backup-\(UUID().uuidString).json"), options: .atomic)
        }
    ...
    func poll(mode: CodexTaskPreviewMode) -> (tasks: [CodexTask], completed: [CodexTask]) {
        ...
        do {
            ...
            for file in files where !seen.contains(file.lastPathComponent) {
