...
final class ClaudeAccountStore: ObservableObject {
    ...
    private func register(_ identity: ClaudeAccountIdentity, snapshot: ClaudeLoginSnapshot, replacing: ClaudeAccountProfile? = nil) throws -> String {
        ...
        try ClaudeAccountBackend.writeSecret(JSONEncoder().encode(snapshot), service: ClaudeAccountBackend.vaultService, account: id)
    ...
    func switchAccount(_ profile: ClaudeAccountProfile, hasRunningTasks: Bool) {
        ...
        perform { executable in
            guard !self.needsRecovery else { throw ClaudeAccountError("请先恢复未完成的切换") }
            try await ClaudeAccountBackend.requireNoClaudeProcesses()
            guard let saved = try ClaudeAccountBackend.readSecret(service: ClaudeAccountBackend.vaultService, account: profile.id) else {
            ...
            try ClaudeAccountBackend.writeSecret(JSONEncoder().encode(source), service: ClaudeAccountBackend.vaultService, account: backupID)
    ...
    func recover() {
        perform { _ in
            ...
            guard let data = try ClaudeAccountBackend.readSecret(service: ClaudeAccountBackend.vaultService, account: id) else { throw ClaudeAccountError("未找到恢复缓存") }
